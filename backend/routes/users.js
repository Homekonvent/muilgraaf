...
router.post("/register", (req, res) => {
    ...
    newdb.exec("insert into users (mail,first_naam,last_name,home,admin, hash) values ('" + json.email + "','tibo','vanheule',0,0,'" + hash + "');COMMIT;", (r, err) => {
        if (err) {
            res.send(err);
            return;
        }
...
router.post("/login", (req, res) => {
    ...
    let hash = crypt.sha512crypt(json.password, "saltsalt");
    newdb.exec("select * from users where email = " + json.email , (r, err) => {
        if (err) {
            res.send(err);
            return;
        }
